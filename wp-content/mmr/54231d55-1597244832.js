...
jQuery(function($) {
	...
	$(window).scroll(function() {
		if ($(this).scrollTop() > 1){  
			$('.site-header, .imothm-content').addClass("fixed-on-scroll");
		} else {
			$('.site-header, .imothm-content').removeClass("fixed-on-scroll");
		}
	});   
	// Hide tag section if untagged
	$('.imothm-tags:contains(Untagged)').css('display', 'none');
	// Smooth out the scroll to top
	$('.back-up div, .back-up span').on('click', function() {
